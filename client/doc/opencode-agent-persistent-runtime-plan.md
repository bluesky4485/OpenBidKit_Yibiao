...
2. `startIsolatedOpenCodeServer()` 使用任务 workspace 作为 OpenCode 进程 `cwd`。
...
### 8.1 拆分原 `startIsolatedOpenCodeServer()`
...
opencodeServerRunner.cjs
  ...
  └─ legacy startIsolatedOpenCodeServer() 可删除或仅测试保留
```
...
默认任务结束后清空 `service/workspace` 的子项，但保留 `service/workspace` 根目录作为常驻 cwd。开发者模式可允许 `keep_runtime` 保留 staging 副本，但不能把多个任务目录留在 OpenCode cwd 内。
...
- 参数从 `taskId/workspaceDir/keepRuntime` 改成 `runtimeRoot/serviceWorkspaceDir`。
- AI proxy 只启动一次。
...
保留或删除旧 `startIsolatedOpenCodeServer()`。本次业务路径不能再调用旧函数。
...
- 删除 `runTask()` 中直接调用 `startIsolatedOpenCodeServer()` 的路径。
...
- 搜索确认 `agentService.cjs` 不再调用 `startIsolatedOpenCodeServer()`。
